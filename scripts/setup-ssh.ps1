...
#
# USAGE:
# 1. Copy your SSH public key to C:\temp\id_ed25519_homelab.pub (or modify path below)
# 2. Open PowerShell as Administrator
# 3. Run: powershell -ExecutionPolicy Bypass -File .\setup-ssh.ps1
#
# =============================================================================
...
Write-Info "LM Studio Port: $LMStudioPort"
...
# =============================================================================
# Step 9: Create Windows Firewall rules
# =============================================================================
...
try {
    ...
    if ($rule) {
        ...
        Write-Success "SSH firewall rule created for port $SSHPort"
    }
} catch {
    Write-Warning "Could not create SSH firewall rule: $_"
    Write-Info "You may need to create the rule manually"
}

# LM Studio API firewall rule
try {
    $rule = Get-NetFirewallRule -DisplayName "LM Studio API" -ErrorAction SilentlyContinue
    if ($rule) {
        Write-Success "LM Studio firewall rule already exists"
    } else {
        New-NetFirewallRule -DisplayName "LM Studio API" `
            -Direction Inbound `
            -Action Allow `
            -Protocol TCP `
            -LocalPort $LMStudioPort `
            -ErrorAction Stop | Out-Null
        Write-Success "LM Studio firewall rule created for port $LMStudioPort"
    }
} catch {
    Write-Warning "Could not create LM Studio firewall rule: $_"
    Write-Info "You may need to create the rule manually"
}
...
Write-Host "================================================================================" -ForegroundColor Green
Write-Host ""
Write-Info "SSH and LM Studio firewall rules configured on this Windows system"
Write-Host ""
...
Write-Host ""
Write-Host "LM Studio API accessible at:" -ForegroundColor Cyan
Write-Host "  http://<this-host-ip>:$LMStudioPort/v1/models" -ForegroundColor Yellow
Write-Host "  (Ensure LM Studio server is bound to 0.0.0.0, not localhost)" -ForegroundColor Yellow
Write-Host ""
